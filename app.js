...
// Generamos una disposición aleatoria de bricks para el nivel 2
const level2Bricks = [];

for (let c = 0; c < brickColumnCount; c++) {
  level2Bricks[c] = []; // inicializamos con un array vacio
  for (let r = 0; r < brickRowCount; r++) {
    // calculamos la posicion del ladrillo en la pantalla
    const brickX = c * (brickWidth + brickPadding) + brickOffsetLeft;
    const brickY = r * (brickHeight + brickPadding) + brickOffsetTop;
...
for (let i = 0; i < 20; i++) {
  ...
  if (level2Bricks[randomColumn][randomRow].status === BRICK_STATUS.ACTIVE) {
    level2Bricks[randomColumn][randomRow].status = BRICK_STATUS.DESTROYED;
  } else {
    i--; // si ya existe un brick en ese puesto, volvemos a intentarlo
  }
}
...
function nextLevel() {
  ...
  // Por ejemplo, puedes incrementar un contador de nivel y cargar el nivel correspondiente
  currentLevel++;
